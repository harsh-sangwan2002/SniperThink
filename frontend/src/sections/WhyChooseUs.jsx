  ...
  return (
    <section className="bg-black text-white py-20 px-6">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
        ...
        {/* Left Content */}
        <motion.div
          className="flex-1 bg-purple-800 rounded-2xl p-10 shadow-xl"
          {...cardHoverEffect}
        >
          <h2 className="text-4xl font-bold mb-4 text-teal-400">Why Choose SniperThink?</h2>
          ...
          <ul className="mb-8 space-y-3">
            {points.map((point, idx) => (
              <li key={idx} className="flex items-start text-white text-sm">
                <CheckCircleIcon className="text-green-400 mr-2 mt-[2px]" />
                {point}
              </li>
            ))}
          </ul>
          <button className="bg-teal-600 hover:bg-teal-800 cursor-pointer transition px-6 py-3 rounded-lg text-white font-semibold shadow-md">
            Get Started Now
          </button>
        </motion.div>
